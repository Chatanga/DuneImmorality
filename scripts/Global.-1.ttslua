...
_ = require("Core").registerLoadablePart(function(savedData)
    ...
    marker_combat_pos = {}
    local origin = getObjectFromGUID("1d4424").getPosition()
    for i = 0, 19 do
    ...
    local zone_vp_combat = getObjectFromGUID("740624").getPosition()
    pos_vp_encours = {
...
function RevealOK(_, color)
    ...
    if helperModule.getPrestigeBonus(color) > 0 then
        i = i + 1
    end

    for _, object in ipairs(player_hand_table) do
        if object.hasTag('Imperium') then
            Wait.time(function()
                object.setPosition(helperModule.getRevealCardPosition(color, i))
                i = i + 1
            end, t)
            t = t + 0.25
        end
...
function CountForce(name)
    ...
    if helperModule.hasHighCouncilSeat(name) and helperModule.hasTech(name, "restricted_ordnance") then
        force = force + 4
    end
...
function addTroopDefense(controlableSpace)
    local joueur = nil
    for _, obj in ipairs(controlableSpace.zone.getObjects()) do
...
function onObjectEnterScriptingZone(zone, enter_object)
    ...
    for faction, zone_reput in pairs(zone_reputs) do
        if zone.guid == zone_reput.guid then
    ...
    if zone.guid == zone_conflict_encours.guid then
        ...
        if round_start == 1 then
            ...
            local controlableSpace = getControlableSpace(enter_object)
            if controlableSpace then
                addTroopDefense(controlableSpace)
            end
        end
    end
...
function Friendship(faction, color)
    ...
    if not hasFriendship then
        ...
        if helperModule.getLeaderName(color) == "Tessia Vernius" then
            local mainBoard = getObjectFromGUID('2da390')
            mainBoard.call("recallSnooper", {factionName = faction, color = color})
        end
    end
end
...
function getControlableSpace(vpToken)
...
function onPlayerTurn(player, previous_player)
    ...
    if bufferedNextPlayer then
        log("Supersede " .. bufferedNextPlayer .. " by " .. tostring(player and player.color))
    end
    bufferedNextPlayer = player and player.color
    Wait.frames(function ()
        ...
        for i, color in ipairs(indexed_colors) do
            ...
            if bufferedNextPlayer == color then
                effect_index = i
            end
            local board = constants.players[color].board
